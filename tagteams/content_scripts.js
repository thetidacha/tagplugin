    ...
    // Function
        ...
        function validationCase(_datatemp = {}, panel) {
            ...
            if(window.location.href.includes("cases.connect.corp.google.com")) {
                ...
                document.querySelectorAll('[id="read-card-tab-panel-case-log"] .message-body-panel').forEach(function(elm) {
                    if(elm.innerText.includes("Emails or feedback from Advertiser/Seller")) {
                        ishas_precall = true;
                    }
                });
                
                if(ishas_precall === false) {
                    lst_notification +=`<p class="_panel__notification_proccess">Not Precall - Checked</p>`;
                    _datatemp.validate_isprecall = "0";
                }
            }
        ...
        function loadInputSettings(_panel) {
            ...
            getChromeStorage('cdtx_settings', (response) => {
                ...
                console.log("Input Setting", _datatemp)
                // Load
                var _object = {};
                if(_datatemp) {
                    _object = _datatemp;
        ...
        function autoLoadCode(keyaction) {
            getChromeStorage('cdtx_settings', (response) => {
                ...
                console.log(_datatemp, "222", _datatemp.auto_loadgtmid);
                // Load
                var _object = {};
                if(_datatemp) {
                    _object = _datatemp;
                ...
                elm.addEventListener('click', function(e){
                    ...
                    if(_action === 'pushto_casesummary') {
                        ...
                        if(case_sumary && location.host.includes('cases.connect.corp.google.com')) {
                            case_sumary.value = panel_div.querySelector('textarea[name="note"]').value;
