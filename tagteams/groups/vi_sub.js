...
function vi_prepareForEmail(isGCC = false){
    wait4Elem('email-address-dropdown material-dropdown-select .address').then(elm => {
        ...
        if(isGCC) {
            ...
            if(emailCC.querySelector('.value')) {
            ...
            wait4Elem('focus-trap [debug-id="email"]').then(item => {
                item.click();
...
function vi_prepareCR(){
    ...
    hotkeys.forEach(function(hotkey) {
        ...
        li.addEventListener('click', function(){
            ...
            wait4Elem('.suggestion-list .list-item .text-segment').then(elm => {
                elm.click();
                divLoading.remove();

                var _composeemailcard = document.querySelector('.write-cards-wrapper:not([style*="display:none"]):not([style*="display: none"]) card.write-card.is-top[card-type="compose"]');
                if(_composeemailcard) {
                    _composeemailcard.querySelector('.is-top .subject').value = `${subjects[key]} cho trường hợp [${caseId}]`;
                    _composeemailcard.querySelector('.is-top .subject').dispatchEvent(new Event('input'));
                }
...
var vi_TagteamFocusCase = () => {
    try {
        ...
        var caseId = document.querySelector('.case-id').innerText;
        var showMoreBtn = document.querySelector('.more-less-button:not(.show-more)');
        if (showMoreBtn) {
                showMoreBtn.click()
        }
        var hiddenPhone = document.querySelector("span[aria-label='View hidden phone number']");
        if (hiddenPhone) {
                hiddenPhone.click();
        }
        document.querySelector('.recipient-dropdown > dropdown-button .button-text').click();
        var caseSumary = document.querySelector('[aria-label="Enter a case summary"]');
        caseSumary.style.backgroundColor = '#FFFC22';
        caseSumary.style.fontWeight = 'bold';
            ...
            if(field == 'Appointment Time') {
                var _thischeck_time = function(stt = 1) {
                    ...
                    if(appointmentTime == 'Invalid Date') {
                        ...
                        window.tempAppointmentTime = window.tempAppointmentTime || {};
                        window.tempAppointmentTime = matchingElement.innerText;
        ...
        wait4Elem('pii-value[debug-id="pii-value"] .value:not(.copied-to)').then((elm) => {
